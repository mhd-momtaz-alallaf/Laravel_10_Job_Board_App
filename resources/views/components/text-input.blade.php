<div class="relative">
    @if ($formId) {{-- if the formId is passed in the index form --}}
        <button type="button" class="absolute top-0 right-0 flex h-full items-center pr-2"
            onclick="document.getElementById('{{ $name }}').value = ''; document.getElementById('{{ $formId }}').submit();"> {{-- first javaScript satatement is to reset the input, the second one to submit the form --}}
